...
-- --------------------------------------------------
-- Date Created: 02/03/2022 08:05:32
-- Generated from EDMX file: C:\Users\essed\OneDrive\Bureau\PROJETS_A_RENDRE\CSharpL3\code\GestionCommndesNaza\models\Model1.edmx
-- --------------------------------------------------
...
GO
IF OBJECT_ID(N'[dbo].[FK_OrderIvoice]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[Ivoices] DROP CONSTRAINT [FK_OrderIvoice];
GO
IF OBJECT_ID(N'[dbo].[FK_OrderPayment]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[Payments] DROP CONSTRAINT [FK_OrderPayment];
GO
...
GO
IF OBJECT_ID(N'[dbo].[Ivoices]', 'U') IS NOT NULL
    DROP TABLE [dbo].[Ivoices];
GO
...
-- Creating table 'Ivoices'
CREATE TABLE [dbo].[Ivoices] (
    [Id] int IDENTITY(1,1) NOT NULL,
    [Numero] nvarchar(max)  NOT NULL,
    [Order_Id] int  NULL
);
...
-- Creating primary key on [Id] in table 'Ivoices'
ALTER TABLE [dbo].[Ivoices]
ADD CONSTRAINT [PK_Ivoices]
    PRIMARY KEY CLUSTERED ([Id] ASC);
GO
...
-- Creating foreign key on [Order_Id] in table 'Ivoices'
ALTER TABLE [dbo].[Ivoices]
ADD CONSTRAINT [FK_OrderIvoice]
    FOREIGN KEY ([Order_Id])
    REFERENCES [dbo].[Orders]
        ([Id])
    ON DELETE NO ACTION ON UPDATE NO ACTION;
...
-- Creating non-clustered index for FOREIGN KEY 'FK_OrderIvoice'
CREATE INDEX [IX_FK_OrderIvoice]
ON [dbo].[Ivoices]
    ([Order_Id]);
GO
